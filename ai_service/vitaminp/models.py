...
class VitaminPSyn(nn.Module):
    """
    Vitamin-P Syn: Dual-Encoder DINOv2 U-Net for H&E + Synthetic MIF pathology images
    
    Architecture:
    - H&E Encoder (3ch): DINOv2 layers 0-1 (separate)
    - Synthetic MIF Encoder (2ch): DINOv2 layers 0-1 (separate)
    - Fusion: Concatenate layer_1 features
    - Shared Encoder: DINOv2 layers 2-3 (shared)
    - 4 Decoders: HE nuclei, HE cell, MIF nuclei, MIF cell
    ...
    Example:
        >>> model = VitaminPSyn(model_size='base', dropout_rate=0.3)
        >>> he_img = torch.randn(2, 3, 512, 512)
        >>> syn_mif_img = torch.randn(2, 2, 512, 512)  # Synthetic MIF from GAN
        >>> outputs = model(he_img, syn_mif_img)
    """
    def __init__(self, model_size='base', dropout_rate=0.3, freeze_backbone=False):
        super().__init__()
        ...
        # ========== SYNTHETIC MIF ENCODER (layers 0-1) ==========
        print(f"Building Synthetic MIF encoder with DINOv2-{model_size}")
        self.mif_backbone = DINOv2Backbone(model_size=model_size, in_channels=2)
        ...
        print(f"✓ VitaminPSyn initialized with {model_size} backbone")
        print(f"  Embed dim: {embed_dim} | Decoder dims: {dec_dims}")
    ...
    def forward(self, he_img, syn_mif_img):
        """
        Forward pass
        
        Args:
            he_img: (B, 3, H, W) - H&E image
            syn_mif_img: (B, 2, H, W) - Synthetic MIF image from GAN
        ...
        if he_img.shape[2] != 518 or he_img.shape[3] != 518:
            he_img = F.interpolate(he_img, size=(518, 518), mode='bilinear', align_corners=False)
        if syn_mif_img.shape[2] != 518 or syn_mif_img.shape[3] != 518:
            syn_mif_img = F.interpolate(syn_mif_img, size=(518, 518), mode='bilinear', align_corners=False)
        ...
        # ========== SYNTHETIC MIF ENCODER (layers 0-1) ==========
        mif_features = self.mif_backbone(syn_mif_img)
        mif_layer_keys = sorted([k for k in mif_features.keys()])
        mif_x0 = mif_features[mif_layer_keys[0]]
        mif_x1 = mif_features[mif_layer_keys[1]]
